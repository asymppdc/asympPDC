...
% [H Z, ps, ps0, converged] = specfactorization_Wilson(S, fs, tol)
...
% modified by James A. Henderson, 2020
% REF:
...
function [H, Z, ps, ps0, converged, relerr] = specfactorization_wilson(S, fs, tol)
	...
	I = eye(k);
	maxiter = min( 20, floor(sqrt(10/tol)) );

	U = zeros(size(Sarr));
    cholflag=0;
	for j = 1 : M
		[temp,flag] = chol(Sarr(:,:,j));     
        if flag==1
            cholflag=1;
            break
        else
            U(:,:,j)=reshape(temp,k,k,1);
        end
	end
