%% GCT_ALG
%        Perform both Granger causality test (GCT) and instantaneous Granger
%        causality test (IGCT) from time series, AR model coefficients and
%        covariance matrix.
%
%% Syntax
%        [Tr_gct, pValue_gct, Tr_igct, pValue_igct] = GCT_ALG(u,A,pf, ...
%                                  gct_signif,igct_signif,flgVerbose)
%
%% Input arguments
%        u          - data
...
if nargin < 4,
   error('GCT_ALG.M requires at least 4 input arguments.');
elseif nargin == 4,
   igct_signif = gct_signif;
   flgVerbose = 0;
elseif nargin == 5
   if igct_signif == 0 || igct_signif == 1
      flgVerbose = igct_signif;
      igct_signif = gct_signif;
   else
      flgVerbose = 0;
   end;
...
%    else
%       fprintf(['\n>>>> There are ' int2str(nPairsIGC) ' pairs of channels with'])
%       fprintf('\n      significant Instantaneous Causality.\n')
%    end
%    fprintf('\n')
%    disp(repmat('=',1,100))
% end
